      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="flex gap-2 mb-4">
            <button onClick={() => setPlatformFilter("")} className={`text-xs px-2.5 py-1 rounded ${!platformFilter ? "bg-lime-400/20 text-lime-400" : "text-zinc-500 hover:text-zinc-300"}`}>All</button>
            {platforms.map((p) => (
              <button key={p} onClick={() => setPlatformFilter(p)} className={`text-xs px-2.5 py-1 rounded ${platformFilter === p ? "bg-lime-400/20 text-lime-400" : "text-zinc-500 hover:text-zinc-300"}`}>{platformIcon(p)} {p}</button>
            ))}
          </div>
            ...
            {filteredSubs.map((sub) => (
              <div key={sub.id} className="rounded-xl border border-white/[0.04] bg-white/[0.015] backdrop-blur-[2px] p-4">
                <div className="flex items-start justify-between">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <Link href={`/admin/campaigns/${sub.campaign_id}/submissions/${sub.id}`} className="text-sm text-lime-400 hover:underline">#{sub.id}</Link>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {sub.status !== "paid" && (
                      <button onClick={() => { setPayoutSubId(sub.id); setPayoutAmount(sub.est_earnings.toFixed(2)); setPayoutOpen(true) }} className="text-lime-400 hover:text-lime-300 transition-colors">
                ...
                <div className="grid grid-cols-4 gap-2 mt-3">
                  ...
                  <div><p className="text-[10px] text-zinc-600 uppercase">Earnings</p><p className="text-sm font-mono text-lime-400">{formatCurrency(sub.est_earnings)}</p></div>
                </div>
              </div>
            ))}
          </div>
                  ...
                  <tbody>
                    {creator.payment_history.map((p, i) => (
                      <tr key={i} className="border-b border-white/[0.03]">
                        ...
                        <td className="px-4 py-3 text-sm text-right font-mono text-lime-400">{formatCurrency(p.amount)}</td>
          ...
          <div className="rounded-xl border border-white/[0.04] bg-[#0a0a0a] backdrop-blur-md w-full max-w-md p-6">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input type="number" step="0.01" value={payoutAmount} onChange={(e) => setPayoutAmount(e.target.value)} className="w-full bg-white/[0.05] border border-white/[0.08] text-zinc-100 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-lime-400/30" />
              </div>
              <div>
                <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-1 block">Method</label>
                <select value={payoutMethod} onChange={(e) => setPayoutMethod(e.target.value)} className="w-full bg-white/[0.05] border border-white/[0.08] text-zinc-100 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-lime-400/30">
              ...
              <div>
                ...
                <input value={payoutRef} onChange={(e) => setPayoutRef(e.target.value)} placeholder="Transaction ID" className="w-full bg-white/[0.05] border border-white/[0.08] text-zinc-100 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-lime-400/30" />
              </div>
            </div>
            <div className="flex justify-end gap-2 mt-4">
              ...
              <button onClick={handlePayout} className="bg-lime-400 text-black font-extrabold text-xs px-6 py-2 rounded-lg uppercase tracking-wide shadow-[0_0_25px_-5px_rgba(163,230,53,0.4)]">Log Payment</button>
            </div>
          </div>
        </div>
      )}
