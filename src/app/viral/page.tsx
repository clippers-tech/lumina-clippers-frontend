...
export default function ViralLoginPage() {
  ...
  const handleSendCode = async () => {
    if (!email) {
      toast({ description: "Enter your email first", variant: "warning" })
      return
    }
    setOtpSending(true)
    try {
      await clipperAuth.sendCode(email)
      setOtpStep(true)
      toast({ description: "Code sent to your email!", variant: "success" })
    } catch (err) {
      toast({
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      const result = await clipperAuth.verifyCode(email, otpCode)
      setClipperToken(result.access_token)
      toast({ description: "Login successful!", variant: "success" })
      router.push("/clipper/dashboard")
    } catch (err) {
