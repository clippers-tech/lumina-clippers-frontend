...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  const isLoginPage = pathname === "/cx0-auth-8f3a"
          ...
          <nav className="border-b border-white/[0.06] bg-[#0b2518]/80 backdrop-blur-md sticky top-0 z-40">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex items-center justify-between h-14">
                <Link href="/cx0-auth-8f3a/dashboard" className="flex items-center gap-2">
                  <LuminaLogo size={28} />
                  <span className="font-bold text-sm uppercase tracking-wider text-zinc-100 hidden sm:inline">Lumina Clippers</span>
                  <span className="font-bold text-xs uppercase tracking-wider text-zinc-100 sm:hidden">Client</span>
                </Link>
                <div className="flex items-center gap-2 sm:gap-4">
                  ...
                  <button
                    onClick={() => { clearToken(); router.push("/cx0-auth-8f3a") }}
